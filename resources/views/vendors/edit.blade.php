...
<div class="container-fluid">
	{!! Form::open(['method'=>'patch','route'=>['vendor.update',$vendor->id]]) !!}
		...
		<bootstrap-card use-header = "true" use-body="true" use-footer = "true">
			...
			<template slot = "body">
	            ...
	            <div class="content">
	                ...
	                <!-- Api Key -->
	                <bootstrap-input class="form-group-no-border {{ $errors->has('api_key') ? ' has-danger' : '' }}" 
	                    use-label = "true"
	 					label = "Api Key"
	                    b-placeholder="Api Key Access"
	                    b-name="api_key"
	                    b-type="text"
	                    b-value="{{ old('api_key') ? old('api_key') : $vendor->api_key }}"
	                    b-err="{{ $errors->has('api_key') }}"
	                    b-error="{{ $errors->first('api_key') }}"
	                    >
	                </bootstrap-input>

	                <!-- Api Token -->
	                <bootstrap-input class="form-group-no-border {{ $errors->has('api_token') ? ' has-danger' : '' }}" 
	                    use-label = "true"
	 					label = "Api Token"
	                    b-placeholder="Api Token"
	                    b-name="api_token"
	                    b-type="text"
	                    b-value="{{ old('api_token') ? old('api_token') :  $vendor->api_token }}"
	                    b-err="{{ $errors->has('api_token') }}"
	                    b-error="{{ $errors->first('api_token') }}"
	                    >
	                </bootstrap-input>
