    ...
    public function prepareTableSelectRows($rows)
    {
        ...
        if (isset($rows)) {
            foreach ($rows as $key => $value) {
                ...
                $state = $value->state;
                $zipcode = $value->zipcode;
                $full_name = (empty((array)$nick_name) || is_null($nick_name)) ? $name : $name.' ('.$nick_name.')';
                // create full name here
                $rows[$key]['full_name'] = $full_name;
                ...
                // append last column to table here
                $last_column = '<button class="vendor-selected btn btn-success btn-sm" vendor_id="'.$value->id.'" vendor_name="'.$full_name.'">Select</button>';
                $last_column .= '</div>';
                $rows[$key]['action'] = $last_column;
            }
        }
